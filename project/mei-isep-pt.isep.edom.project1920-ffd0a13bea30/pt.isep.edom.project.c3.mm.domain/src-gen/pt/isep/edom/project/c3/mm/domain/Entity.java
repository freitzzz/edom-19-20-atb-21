 ...
 * @model annotation="http://www.eclipse.org/emf/2002/Ecore constraints='NameCannotBeNull NameLengthMustBeEqualOrGreaterThanThree MustHaveAtLeastIdAndNameFields CannotHaveDuplicatedFields CannotHaveSubEntityWithSameNameAsAnEntityName CannotHaveSubEntityWithSameName'"
 *        annotation="http://www.eclipse.org/emf/2002/Ecore/OCL/Pivot NameCannotBeNull='not self.name.oclIsUndefined()' NameLengthMustBeEqualOrGreaterThanThree='self.name.size() &gt;= 3' MustHaveAtLeastIdAndNameFields='\n\t\t\tself.fields -&gt; select(field : Field | field.name = \'id\' or field.name = \'name\') -&gt; size() = 2\n\t\t' CannotHaveDuplicatedFields='\n\t\t\tself.fields -&gt; collect(field : Field | field.name) -&gt; asSet() -&gt; size() = self.fields -&gt; size()\n\t\t' CannotHaveSubEntityWithSameNameAsAnEntityName='\n\t\t\tself.subentities -&gt; collect(subentity: SubEntity | subentity.name)\n\t\t\t-&gt; intersection(Entity.allInstances() -&gt; collect(entity: Entity | entity.name))\n\t\t\t-&gt; isEmpty()\n\t\t' CannotHaveSubEntityWithSameName='\n\t\t\tself.subentities -&gt; collect(subentity: SubEntity | subentity.name)\n\t\t\t-&gt; asSet() -&gt; size()\n\t\t\t=\n\t\t\tself.subentities -&gt; size()\n\t\t'"
 * @generated
 */
